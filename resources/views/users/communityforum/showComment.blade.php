...
@section('content')
    <div class="container"> 
            ...
            @if(is_iterable($comments) && count($comments) > 0)
                @forelse ($comments as $comment)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('updateComment', [$communityforums->id, $comment->id]) }}" class="btn btn-warning">Edit Comment</a>
                            <form method="post" action="{{ route('deleteComment', [$communityforums->id, $communityforum->id]) }}" style="display: inline;">
                                @csrf
                                @method('DELETE')
        ...
        <a href="{{ route('communityforum') }}" class="btn btn-secondary mt-3">Back</a>
    </div>
@endsection
